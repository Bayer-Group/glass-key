 ...
 */
object ResourceOwnerControllerCF extends SampleController {

  override def env: PlayClientRuntimeEnvironment = PlayClientRuntimeEnvironment(
  ...
  def doSomething: WSResponse => Result = { response: WSResponse =>
  ...
  override def index(name: String) = Action.async { implicit request =>
    doAction(
      request,
      "https://api.cf.company.com/v2/users?results-per-page=20&page=1",
      httpMethod,
      doSomething)
  }
}
