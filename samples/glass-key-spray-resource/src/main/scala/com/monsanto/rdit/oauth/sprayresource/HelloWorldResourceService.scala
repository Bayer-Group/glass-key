...
class HelloWorldResourceService(var actorRefFactory: ActorRefFactory) extends HttpService with SprayJsonSupport {
  ...
  implicit val env = SprayResourceRuntimeEnvironment("hello-validation-client", new OAuthConfig.Default())
  val authenticator = new BearerTokenAuthenticator.Default()
                ...
                get { ctx =>
                  ctx.complete {
                    restrictedUser.user_id match {
                      ...
                      case None => HelloModelReturn(Some(1), Some("Hello client with id = " + restrictedUser.client_id.get + ", you said your name was " + name), None, None)
                    }
                  }
                }
              }
